...
class elFinder {
	...
	protected function rename($args) {
		...
		if(!$this->checkExtension($name)) {
			return array('error' => $this->error(self::ERROR_RENAME, $name, "New name not allowed"));
		}
	...
	protected function upload($args) {
		...
		foreach ($files['name'] as $i => $name) {
			...
			if(!$this->checkExtension($name)){
				$result['warning'] = $this->error(self::ERROR_UPLOAD_FILE, $name, "Not allowed upload this file");
			...
			$extensionFile = explode('.', $name)[1];
			$newNameFile = $this->generateRandomString();
			$newFileName = $newNameFile.".".$extensionFile;

			if (($file = $volume->upload($fp, $target, $newFileName, $tmpname)) === false) {
	...
	protected function checkExtension($name) {
		...
		if(count($name) != 2){
			...
			if(in_array($name[1], $this->_options['extensionAllow'])){
				return true;
			}else{
				return false;
			}

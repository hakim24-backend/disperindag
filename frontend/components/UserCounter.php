...
class UserCounter extends Component
{
	...
	protected $hits_total=0;
	protected $hits_today=0;
	/**
	 *
	 */
	...
	public function refresh($force = false)
	{
		...
		if ($this->isNewDay()) {
			...
			$lastUpdateTotalHits = $this->countHitsToday();
			...
			$this->update($this->tableSave, array('save_value' => $this->hits_total + $lastUpdateTotalHits), 'save_name = "hits"');
	 ...
	 * Returns the number of users which are online at the moment.
	 */
	protected function countHitsToday()
	{
        $count = (new \yii\db\Query())
            ->select(array('sum(visits) AS hits_count'))
            ->from($this->tableUsers);
	...
	protected function insertOrUpdateIpAddress()
	{
		...
		/*$count = (new \yii\db\Query())
            ->select(array('visits'))
            ->from($this->tableUsers)
            ->where(['user_ip'=>$hashedIpAddress])->one();
         echo $count['visits'];*/

		$sql = 'INSERT INTO ' . $this->tableUsers . ' VALUES (:ipAddress, :time, :hits) ON DUPLICATE KEY UPDATE user_time = :time, visits= visits+1';
		Yii::$app->db->createCommand($sql)
            ->bindParam(':ipAddress', $hashedIpAddress, \PDO::PARAM_STR)
            ->bindParam(':time', $currentTimestamp, \PDO::PARAM_INT)
            ->bindParam(':hits', $hits, \PDO::PARAM_INT)
            ->execute();
	}
	 ...
	 * @return date
	 */
	public function getHitsToday()
	{
		return $this->hits_today;
	}
